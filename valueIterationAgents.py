...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, prob in nextStatesAndProbs:
            ...
            #     QValue = self.mdp.getReward(state, action, nextState)
            # else:
            reward = self.mdp.getReward(state, action, nextState)
            # print("reward: " + str(reward))
            valueNextState = self.getValue(nextState)
            QValue += prob * (reward + self.discount * valueNextState)
        return QValue
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for state in states:
            if not self.mdp.isTerminal(state):
                actionsFromState = self.mdp.getPossibleActions(state)
                for action in actionsFromState:
                    nextStatesAndProbs = self.mdp.getTransitionStatesAndProbs(state, action)
                    for (nextState, prob) in nextStatesAndProbs:
                        if prob > 0:
                            statesAndPredecessors[nextState].add(state)
        ...
        for i in range(self.iterations):
            if queue.isEmpty():
                ...
                if not self.mdp.isTerminal(state):
                    ...
                    predecessors = statesAndPredecessors[state]
                    # print(predecessors)]
                    for pred in predecessors:
                        bestAction = self.getAction(pred)
                        bestValue = self.computeQValueFromValues(pred, bestAction)
                        diff = abs(self.values[pred] - bestValue)
                        if diff > self.theta:
                            queue.update(pred, -1 * diff)
